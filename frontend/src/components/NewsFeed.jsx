  ...
  return (
    <div className="sentiment-section">
      <div className="sentiment-bar">
        <div className="sb-pos" style={{ flex: positive }} title={`Positive ${positive}%`} />
        <div className="sb-neu" style={{ flex: neutral  }} title={`Neutral ${neutral}%`}  />
        <div className="sb-neg" style={{ flex: negative }} title={`Negative ${negative}%`} />
      </div>
      <div className="sentiment-legend">
        <span className="sleg-item"><span className="sleg-dot pos" />{positive.toFixed(0)}% Positive</span>
        <span className="sleg-item"><span className="sleg-dot neu" />{neutral.toFixed(0)}% Neutral</span>
        <span className="sleg-item"><span className="sleg-dot neg" />{negative.toFixed(0)}% Negative</span>
      </div>
    </div>
  );
}
...
function NewsCard({ article }) {
  ...
  const sentiment = tags.find(t => ["positive","negative","neutral"].includes(t)) || "neutral";
...
export default function NewsFeed({ symbol }) {
  ...
  const filtered = filter === "All"
    ? news
    : news.filter(n => n.tags?.some(t => t.toLowerCase() === filter.toLowerCase()));
  ...
  return (
    <div className="news-section">
      ...
      {loading ? (
        ...
        <div className="empty-state">No {filter !== "All" ? filter.toLowerCase() : ""} news found.</div>
      ) : (
        <div className="news-grid">
