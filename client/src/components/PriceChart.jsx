...
const LARGE_TF = ["3M", "6M", "1Y", "ALL"]; // auto-switch to line
const MAX_VISIBLE = 120; // max candles in main viewport
...
export default function PriceChart({ symbol, theme }) {
  ...
  // Auto-switch chart type on large timeframes
  useEffect(() => {
    setChartType(LARGE_TF.includes(timeframe) ? "line" : "candle");
  }, [timeframe]);

  // Reset viewport end when data loads
  useEffect(() => {
  ...
  }, [data, theme, chartType, start, end]); // eslint-disable-line
  ...
  return (
    <div className="chart-card">
      ...
      {/* Minimap — only when dataset is large */}
      {total > MAX_VISIBLE && (
        <canvas
          ref={mapRef}
          style={{
