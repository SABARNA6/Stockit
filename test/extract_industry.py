#!/usr/bin/env python3
"""
Extract Symbol and Industry mapping from data.csv
...
def fetch_industry(item: tuple) -> dict:
    """Fetch industry info for a symbol."""
    symbol, idx = item
    global _counter
    ...
    try:
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            if reader.fieldnames and 'SYMBOL' in reader.fieldnames:
                ...
                f.seek(0)
                symbols = [row.strip() for row in f.readlines() if row.strip() and not row.startswith('SYMBOL')]
    except Exception as e:
        print(f"❌ Error reading data.csv: {e}")
        sys.exit(1)
    
    _total = len(symbols)
    print(f"📊 Fetching industries for {_total} symbols...")
